  ...
  <section id="ai-services" aria-labelledby="ai-services-heading" className="ai-services-section section-space text-white">
    <div className="section-shell">
      <div className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
        <div>
          <p className="inline-flex items-center gap-2 rounded-full bg-primary-500 px-4 py-2.5 text-xs font-bold uppercase tracking-[0.08em] text-amazon-dark"><Cpu className="h-4 w-4 shrink-0" aria-hidden="true" />02 / AI solutions for your business</p>
          <h2 id="ai-services-heading" className="mt-6 max-w-xl text-4xl font-bold leading-[1.08] tracking-tight md:text-5xl">AI solutions that drive real <span className="text-primary-400">business growth.</span></h2>
      ...
      <div id="ai-service-grid" className="mt-12 scroll-mt-28 border-t border-primary-400/20 pt-10 md:mt-16">
        <div className="mb-8 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <h3 className="text-2xl font-bold tracking-tight">Six ways to put AI to work.</h3>
          ...
          {AI_SERVICES.map((service, index) => {
            ...
            return (
              <article key={service.id} id={`ai-${service.id}`} data-ai-service={service.id} aria-labelledby={`ai-title-${service.id}`} className="ai-service-card scroll-mt-28">
                ...
                <h4 id={`ai-title-${service.id}`} className="ai-service-title">{service.title}</h4>
      ...
      <div className="mt-8 flex flex-col items-start justify-between gap-5 rounded-2xl bg-primary-500 p-6 text-amazon-dark sm:flex-row sm:items-center md:p-8">
        <div><h3 className="text-xl font-bold">Have a challenge in mind?</h3><p className="mt-2 max-w-2xl text-sm leading-relaxed">Tell us what you want to improve. We’ll help you explore a practical starting point for AI.</p></div>
